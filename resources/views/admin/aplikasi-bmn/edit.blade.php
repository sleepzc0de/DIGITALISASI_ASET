        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <div class="flex items-center space-x-3">
                    ...
                    <h2 class="font-bold text-3xl text-gray-900 tracking-tight">Edit Aplikasi</h2>
                </div>
                <p class="text-gray-600 mt-1">Perbarui data aplikasi {{ $aplikasiBmn->nama_aplikasi }}</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <form action="{{ route('admin.aplikasi-bmn.update', $aplikasiBmn) }}" method="POST" enctype="multipart/form-data" data-aos="fade-up">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-8 py-6 border-b border-gray-200">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-gray-900">Edit Informasi Aplikasi</h3>
                            <p class="text-gray-600 text-sm">Perbarui data aplikasi sesuai kebutuhan</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8">
                    ...
                    <div class="mb-8">
                        <label class="block text-sm font-medium text-gray-900 mb-2">Logo Saat Ini</label>
                        <div class="flex items-center space-x-4">
                            <div class="h-24 w-24 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center overflow-hidden">
                                <img src="{{ $aplikasiBmn->getLogoUrl() }}" alt="{{ $aplikasiBmn->nama_aplikasi }}" class="h-16 w-16 object-contain">
                            </div>
                            <div class="text-sm text-gray-600">
                                <p>Logo aplikasi saat ini. Upload logo baru untuk mengganti.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama_aplikasi" value="{{ old('nama_aplikasi', $aplikasiBmn->nama_aplikasi) }}" required
                            ...
                            <div>
                                ...
                                <select name="kategori" required
                                    ...
                                    <option value="BMN" {{ old('kategori', $aplikasiBmn->kategori) == 'BMN' ? 'selected' : '' }}>BMN</option>
                                    <option value="Pengadaan" {{ old('kategori', $aplikasiBmn->kategori) == 'Pengadaan' ? 'selected' : '' }}>Pengadaan</option>
                                    <option value="Inventaris" {{ old('kategori', $aplikasiBmn->kategori) == 'Inventaris' ? 'selected' : '' }}>Inventaris</option>
                                    <option value="Monitoring" {{ old('kategori', $aplikasiBmn->kategori) == 'Monitoring' ? 'selected' : '' }}>Monitoring</option>
                                </select>
                                @error('kategori')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="url" name="url_aplikasi" value="{{ old('url_aplikasi', $aplikasiBmn->url_aplikasi) }}"
                                       class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200"
                            ...
                            <div>
                                ...
                                <select name="status" required
                                        class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                                    <option value="Aktif" {{ old('status', $aplikasiBmn->status) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                                    <option value="Maintenance" {{ old('status', $aplikasiBmn->status) == 'Maintenance' ? 'selected' : '' }}>Maintenance</option>
                                    <option value="Non-Aktif" {{ old('status', $aplikasiBmn->status) == 'Non-Aktif' ? 'selected' : '' }}>Non-Aktif</option>
                                </select>
                                @error('status')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="space-y-6">
                            <!-- Upload Logo Baru -->
                            <div>
                                <label class="block text-sm font-medium text-gray-900 mb-2">Upload Logo Baru (Opsional)</label>
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah_user" value="{{ old('jumlah_user', $aplikasiBmn->jumlah_user) }}" min="0"
                                       class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                                @error('jumlah_user')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="biaya_lisensi" value="{{ old('biaya_lisensi', $aplikasiBmn->biaya_lisensi) }}" min="0" step="0.01"
                                       class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200"
                                       placeholder="0.00">
                                @error('biaya_lisensi')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="periode_lisensi"
                                    ...
                                    <option value="Tahunan" {{ old('periode_lisensi', $aplikasiBmn->periode_lisensi) == 'Tahunan' ? 'selected' : '' }}>Tahunan</option>
                                    <option value="Bulanan" {{ old('periode_lisensi', $aplikasiBmn->periode_lisensi) == 'Bulanan' ? 'selected' : '' }}>Bulanan</option>
                                    <option value="Selamanya" {{ old('periode_lisensi', $aplikasiBmn->periode_lisensi) == 'Selamanya' ? 'selected' : '' }}>Selamanya</option>
                                </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_implementasi" value="{{ old('tanggal_implementasi', $aplikasiBmn->tanggal_implementasi ? $aplikasiBmn->tanggal_implementasi->format('Y-m-d') : '') }}"
                                   class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_expired" value="{{ old('tanggal_expired', $aplikasiBmn->tanggal_expired ? $aplikasiBmn->tanggal_expired->format('Y-m-d') : '') }}"
                                   class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                    ...
                    <div class="mt-8 pt-8 border-t border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="email" name="pic_email" value="{{ old('pic_email', $aplikasiBmn->pic_email) }}"
                                       class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                                @error('pic_email')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="pic_telepon" value="{{ old('pic_telepon', $aplikasiBmn->pic_telepon) }}"
                                       class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200">
                                @error('pic_telepon')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="mt-8 space-y-8">
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" rows="3"
                                      class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200"
                                      placeholder="Deskripsi singkat tentang aplikasi">{{ old('deskripsi', $aplikasiBmn->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="fitur_utama" rows="4"
                                      class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200"
                                      placeholder="Tuliskan fitur-fitur utama aplikasi">{{ old('fitur_utama', $aplikasiBmn->fitur_utama) }}</textarea>
                            @error('fitur_utama')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
